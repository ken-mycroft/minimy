...
"""
script to read from mic and save utterances 
...
class Audio(object):
    """Streams raw audio from microphone. Data is 
    received in a separate thread, and stored in 
    a buffer, to be read from."""
...
def main():
    """aggressiveness of VAD: an integer between 0 and 3, 0 being
    the least aggressive about filtering out non-speech, 3 the
    ...
    # for now, just grab device index from config file
    # but really need to get all params or its unusable
    # need format, sample rate, etc. Mic should probably
    # have its own cfg file
    cfg = Config()
    #device_indx = get_cfg_val('input_device')
    ...
    # Stream from microphone to file
    wav_data = bytearray()
    for frame in frames:
